            ...
            dispatcher.register(ClientCommandManager.literal("autodisconnect")
                    .then(ClientCommandManager.literal("toggle").executes(context -> {
                        ...
                        context.getSource().sendFeedback(
                                Text.of((toggle ? "Enabled" : "Disabled") + " AutoDisconnect!"));
                        writeFile();
                        return 1;
                    }))
                    .then(ClientCommandManager.literal("health")
                            .then(ClientCommandManager
                                    .argument("health", IntegerArgumentType.integer(1, 19))
                                    .executes(context -> {
                                        healthToLeave = IntegerArgumentType.getInteger(context, "health");
                                        context.getSource().sendFeedback(
                                                Text.of("AutoDisconnect Health changed to "
                                                        + healthToLeave));
                                        writeFile();
                                        return 1;
                                    })))
                    .then(ClientCommandManager.literal("cooldown")
                            .then(ClientCommandManager
                                    .argument("cooldown", IntegerArgumentType.integer(1, 60))
                                    .executes(context -> {
                                        cooldownInSeconds = IntegerArgumentType.getInteger(context, "cooldown");
                                        context.getSource().sendFeedback(
                                                Text.of("AutoDisconnect Cooldown changed to "
                                                        + cooldownInSeconds + " second"
                                                        + (cooldownInSeconds == 1 ? "" : "s")));
                                        writeFile();
                                        return 1;
                                    })))
                    .then(ClientCommandManager.literal("status").executes(context -> {
                        context.getSource().sendFeedback(
                                Text.of("AutoDisconnect is " + (toggle ? "Enabled" : "Disabled")));
                        context.getSource()
                                .sendFeedback(Text.of("AutoDisconnect Health is " + healthToLeave));
                        context.getSource().sendFeedback(
                                Text.of("AutoDisconnect Cooldown is " + cooldownInSeconds
